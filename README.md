...
- 按日期区间读取当天自定义对局，默认“仅显示内战 / 5V5 / 时长大于 8 分钟”，三个过滤开关可单独调整。
- 汇总卡展示总对局、总人数、平均胜率、均分、平均 KDA、蓝红方胜场，以及位置分布、标签统计和评级分布。
- 对局列表以蓝红双方对阵展示，标记全场评分最高的玩家为 MVP，并显示双方评分均分、时长、队列，以及“速推局 / 拉锯局 / 碾压局”标签。
- 展开单局可查看十人明细，包括英雄、召唤师技能、装备、符文或海克斯强化、K/D/A、切 C、伤害、经济、承伤、治疗护盾、伤转与单局评分。
- 玩家评分表按玩家聚合近期内战表现，支持按场次门槛过滤、按任意指标排序、极值高亮和悬停画像卡。
- 表格展示英雄池、场次、胜场、胜率、场均 K/D、KDA、输出 / 前排 / 辅助能力、伤害占比、承伤占比、治疗护盾占比、伤害转化率、标签、高光局、战犯局和伤害 / 承伤 / 助攻三类榜首次数。
- 综合分与评级沿用单局评分和玩家画像体系；高光为单局评分 ≥ 80，战犯为单局评分 < 60，平均胜率按场次加权。
- 提供玩家综合分条形图、英雄出场 TOP、玩家画像雷达，以及“搭档胜率 / 对位胜率”的组队统计。
- 英雄榜与海克斯榜统计出场次数、胜率、最喜欢玩家、胜率最高 / 最低玩家，点击英雄可打开单英雄详情抽屉。
- 对局支持临时删除并在本地持久化，删除后可一键恢复。

> 面向玩家的完整操作说明见 [内战评分使用指南](docs/内战评分使用指南.md)，适合随安装包一同发给朋友。
